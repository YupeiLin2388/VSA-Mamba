...
from datasets.make_data_loader import ChangeDetectionDatset, make_data_loader
from utils_func.metrics import Evaluator
from models.MambaBCD import STMambaBCD
...
class Trainer(object):
    ...
    def training(self):
        ...
        for _ in tqdm(range(elem_num)):
            ...
            ce_loss_1 = 3*F.cross_entropy(output_1, labels, ignore_index=255)
            lovasz_loss = 3*L.lovasz_softmax(F.softmax(output_1, dim=1), labels, ignore=255)
                ...
                if (itera + 1) % 50 == 0:
                    ...
                    if f1_score > best_f1:
                      torch.save(self.deep_model.state_dict(),
                                   os.path.join(self.model_save_path, f'{itera + 1}_{rec:f}_{pre:f}_{f1_score:f}_{iou:f}_{kc:f}_model.pth'))
                      best_f1 = f1_score
...
def main():
    ...
    parser.add_argument('--cfg', type=str, default='./MambaCD/VMamba/classification/configs/vssm1/vssm_base_224.yaml')
    parser.add_argument(
        "--opts",
    ...
    parser.add_argument('--train_dataset_path', type=str, default='./datasets/SYSU/train')
    parser.add_argument('--train_data_list_path', type=str, default='./datasets/SYSU/train_list.txt')
    parser.add_argument('--test_dataset_path', type=str, default='./datasets/SYSU/test')
    parser.add_argument('--test_data_list_path', type=str, default='./datasets/SYSU/test_list.txt')
