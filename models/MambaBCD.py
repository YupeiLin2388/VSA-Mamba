...
from models.ChangeDecoder import ChangeDecoder,ChangeDecoder1
import torch
import torch.nn as nn
import torch.nn.functional as F
...
class STMambaBCD1(nn.Module):
    def __init__(self, pretrained, **kwargs):
        super(STMambaBCD1, self).__init__()
        self.encoder = Backbone_VSSM(out_indices=(0, 1, 2, 3), pretrained=pretrained, **kwargs)
    ...
    def forward(self, pre_data, post_data):
        ...
        # Decoder processing - passing encoder outputs to the decoder
        p1,p2,p3,p4 = self.decoder(pre_features, post_features)

        output = self.main_clf(p1)
        #print('1',p1.shape,output.shape)
        #output = F.interpolate(output, size=pre_data.size()[-2:], mode='bilinear')
        return p1,p2,p3,p4,output
